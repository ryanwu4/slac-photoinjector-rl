...
Usage:
    python -m photoinjector_rl.surrogates.flow.compare_algos \
...
def _run_diffrl(module: str, cfg: Path, args, seed: int, run_dir: Path) -> None:
    ...
    cmd = [sys.executable, "-m", f"photoinjector_rl.surrogates.flow.{module}",
...
def _run_ppo(args, seed: int, run_dir: Path) -> None:
    cmd = [sys.executable, "-m", "photoinjector_rl.surrogates.flow.train_ppo",
