"""
Batched, torch-only, differentiable wrapper around the v1 EmittanceMLP
surrogate. Implements the env API expected by `diffrl.SHAC` and
`diffrl.BPTT` (see `DiffRL/envs/dflex_env.py` for the upstream contract).

State per env i: (knobs_i in [0,1]^5, distgen_i in [0,1]^6, step_count_i).
Action: 5-D direction in [-1, 1]; clipped, scaled by `action_scale`,
added to knobs, clipped back to [0, 1] — gradient flows through the clamp
inside the box.
Reward: -y_norm, where y_norm = surrogate(concat(knobs, distgen)).
...
`torch.no_grad()` when `no_grad=False`, so input gradients propagate from
the reward back to the action that produced the state. This is the
property that makes SHAC/BPTT viable here.
"""
...
class DiffPhotoinjectorEnv:
    """Differentiable batched env over the v1 EmittanceMLP surrogate.
